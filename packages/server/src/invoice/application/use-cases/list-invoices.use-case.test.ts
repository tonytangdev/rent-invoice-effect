...
import { ListInvoicesUseCase, ListInvoicesUseCaseLive } from "./list-invoices.use-case"
...
describe("ListInvoicesUseCase", () => {
  ...
  test("returns invoices with correct pagination", async () => {
    ...
    const mockRepo = Layer.succeed(InvoiceRepository, {
      ...
      findAll: (params) => {
        ...
        const invoices = allInvoices.slice(params.offset, params.offset + params.limit)
  ...
  test("respects limit parameter", async () => {
    ...
    const mockRepo = Layer.succeed(InvoiceRepository, {
      ...
      findById: (id: string) => Effect.fail(new InvoiceNotFoundError({ id })),
      findAll: (params) => {
        const slice = invoices.slice(params.offset, params.offset + params.limit)
